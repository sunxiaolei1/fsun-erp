...
public class MailUtil {
	...
	/** 邮件相关信息 - SMTP 服务器  */
	private Integer mailSMTPHostPort = 465;
	 ...
	 * 设置 SMTP 服务器
	 * @param strSMTPHost 邮件服务器名称或 IP
	 * @param strSMTPHost 邮件服务器的端口
	 * @param strUser 邮件用户名
	 * @param strPassword 密码
	 */
	public void setSMTPHost(String strSMTPHost, Integer port, String strUser, String strPassword) {
		this.mailSMTPHost = strSMTPHost;
	...
	public void setAddress(String strAddress, int iAddressType) {
		switch (iAddressType) {
		case MailUtil.TO: {
			String[] alAddress = StringUtils.stringToArray(strAddress, ";");
	...
	public BaseResult sendBatchSSL() throws GeneralSecurityException{
		...
		return this.sendBatch(session);
	}
	
	public BaseResult sendBatchNoSSL() throws GeneralSecurityException{
		Properties properties = new Properties();
		properties.put("mail.smtp.host", this.mailSMTPHost);
		// 由于国内许多邮箱如网易等需要验证，因此增加验证
		// Session session = Session.getInstance(properties, null);
	 ...
	 */
	public BaseResult sendBatch(Session session) {
		...
		try {
			...
			e.printStackTrace();
			return new BaseResult(Boolean.FALSE,"发送邮箱Address异常!");
		} catch (MessagingException e) {
			e.printStackTrace();
			return new BaseResult(Boolean.FALSE,"发送邮箱Messaging异常!");
		}
		try {
			...
			e.printStackTrace();
			return new BaseResult(Boolean.FALSE,"接受邮箱Messaging异常!");
		}
		try {
			mimemessage.setSubject(this.mailSubject);
		} catch (MessagingException e) {
			e.printStackTrace();
			return new BaseResult(Boolean.FALSE,"设置主题Messaging异常!");
		}
		try {
			mimemessage.setContent(this.mailBody);
		} catch (MessagingException e) {
			e.printStackTrace();
			return new BaseResult(Boolean.FALSE,"设置内容Messaging异常!");
		}
		try {
			mimemessage.setSentDate(new Date());
		} catch (MessagingException e) {
			e.printStackTrace();
			return new BaseResult(Boolean.FALSE,"设置发送日期Messaging异常!");
		}
		Transport transport;
		try {
			transport = session.getTransport("smtp");
			transport.connect(this.mailSMTPHost,this.mailSMTPHostPort, this.mailUser,this.mailPassword);
		} catch (NoSuchProviderException e) {
			e.printStackTrace();
			return new BaseResult(Boolean.FALSE,"获取SMTP时NoSuchProvider异常!");
		} catch (MessagingException e) {
			e.printStackTrace();
			return new BaseResult(Boolean.FALSE,"连接发送邮箱服务器Messaging异常!");
		}
		try {
			Transport.send(mimemessage);
		} catch (MessagingException e) {
			e.printStackTrace();
			return new BaseResult(Boolean.FALSE,"邮件发送Messaging异常!");
		}
