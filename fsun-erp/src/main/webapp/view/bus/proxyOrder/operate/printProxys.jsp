...
/***************************************** 制作送货单的操作 *******************************************/
	...
	function initProxyPagePublicInfo(LODOP, header) {
		...
		LODOP.ADD_PRINT_TEXT(18, "1%", "65%", 20,
			'${applicationScope.companyName}印刷工艺单(代合同)');
		LODOP.SET_PRINT_STYLEA(0, "ItemType", 1);

		LODOP.ADD_PRINT_HTM(52, "1%", "50%", 14, "打印时间：" + now.getFullYear()
				+ "-" + ((now.getMonth() + 1) < 10 ? "0" : "")
		...
		LODOP.ADD_PRINT_TEXT(445, "1%", "90%", 9, "以上各项内容已阅无误,客户签字确认：");
		LODOP.SET_PRINT_STYLEA(0, "ItemType", 1);

		LODOP.SET_PRINT_STYLE("FontSize", 12);
		LODOP.SET_PRINT_STYLE("Bold", 0);
		LODOP.ADD_PRINT_TEXT(445, "60%", "90%", 9, "制单人签字："
				+ header.createManName);
		LODOP.SET_PRINT_STYLEA(0, "ItemType", 1);

		LODOP.SET_PRINT_STYLE("FontSize", 12);
		LODOP.SET_PRINT_STYLE("Bold", 0);
		LODOP.ADD_PRINT_TEXT(465, "1%", "90%", 9, "本加工单三联,白：存根, 红：客户, 黄：印刷机 ");
		LODOP.SET_PRINT_STYLEA(0, "ItemType", 1);
	}
	...
	function createOneProxyPage(LODOP, proxyOrderDto, currRow) {
		...
		LODOP.ADD_PRINT_TABLE(230, "1%", "99%", 210, strStyle
				+ document.getElementById("printTableContextProxy").innerHTML);

		LODOP.SET_PRINT_STYLE("FontSize", 13);
		LODOP.SET_PRINT_STYLE("Bold", 1);
		LODOP.ADD_PRINT_TEXT(20, "70%", "30%", 19, "单号: "
		...
		LODOP.SET_PRINT_STYLE("FontSize", 13);
		LODOP.SET_PRINT_STYLE("Bold", 1);
		LODOP.ADD_PRINT_TEXT(50, "70%", "30%", 19, "日期:   "
		...
		LODOP.ADD_PRINT_HTM(78, "1%", "99%", 150, strStyle
